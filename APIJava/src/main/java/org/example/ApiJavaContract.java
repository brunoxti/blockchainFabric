...
public class ApiJavaContract implements ContractInterface {
    ...
    @Transaction()
    public void createTest(Context ctx, String sampleApiCtxId, String value1) {
        boolean exists = apiJavaExists(ctx,sampleApiCtxId);
        System.out.println("Entrou no metodo");
        if (exists) {
            throw new RuntimeException("The asset "+sampleApiCtxId+" already exists");
        }
        ctx.getStub().putState(sampleApiCtxId, value1.getBytes(UTF_8));
    }

    @Transaction()
    public void calcule(Context ctx, String sampleApiCtxId, String value1, String value2, String operation) {

      try
       {
        int value_1 = Integer.parseInt(value1);
        int value_2 = Integer.parseInt(value2);
        int result = 0;
        String registerOperation = "";
        
        if(operation.equals("+"))
        {
          result = value_1 + value_2;
          registerOperation = value1 + " + " + value2 + " = " + Integer.toString(result);
        }
        ...
        else if(operation.equals("/"))
        {
          if(value1.equals("0") || value2.equals("0")){
            throw new RuntimeException("The value divided by zero");
          }
          result = value_1 / value_2;
          registerOperation = value1 + " / " + value2 + " = " + Integer.toString(result);
        }
          ctx.getStub().putState(sampleApiCtxId, registerOperation.getBytes(UTF_8));
    }
        catch (Exception e) {
